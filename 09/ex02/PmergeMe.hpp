...
private:
	...
	size_t				_binarySearchVector(std::vector<int> &main, int num, size_t start, size_t end);
	std::vector<int>	_insertBsToVector(const std::vector<ab> &pend, std::vector<int> main);
	...
	std::list<int>				_insertBsToList(const std::list<ab> &pend, std::list<int> main);
